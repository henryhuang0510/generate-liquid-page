class ShopifyService {
	...
	async callGraphQL(query, variables = {}, shopDomain = null) {
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					`代理请求失败: ${response.status} - ${errorData.message || response.statusText}`
				);
			}
	...
	async publishSection(sectionName, liquidCode, shopDomain = null) {
		try {
			...
			if (
				...
				result.themeFilesUpsert.userErrors.length > 0
			) {
    ...
    `;
		...
		const productSetInput = templateSuffix ? { ...productSet, templateSuffix } : productSet;
	...
	convertToProductSet(productData) {
		...
		// 从变体中提取选项信息
		const optionValues = [
			...new Set(productData.variants?.map((v) => v.option1).filter(Boolean) || []),
		];
		...
		if (productData.variants && productData.variants.length > 0) {
			productSet.files = productData.variants
				.map((variant) => ({
					alt: variant.featured_image?.alt || variant.title || "",
					contentType: "IMAGE",
					filename: variant.featured_image?.src?.split("/").pop() || "",
	...
	async publishProductToOnlineStore(productId, shopDomain = null) {
      ...
      `;
			...
			if (
				...
				result.publishablePublish.userErrors.length > 0
			) {
	...
	async unpublishProductFromOnlineStore(productId, shopDomain = null) {
      ...
      `;
			...
			if (
				...
				result.publishableUnpublish.userErrors.length > 0
			) {
	...
	async importAndPublishProduct(productSet, templateSuffix = null, shopDomain = null) {
		try {
			...
			// 2. 发布到 Online Store
			const publishResult = await this.publishProductToOnlineStore(
				importResult.product.id,
				shopDomain
			);
